 ...
 * 
 * Copyright (c) 2017 Adrian Wardell Licensed MIT
 * <https://github.com/siddii/angular-timer/blob/master/LICENSE.txt>
 */
  ...
  .directive('timer', ['$compile', function ($compile) {
    return  {
      ...
      controller: ['$scope', '$element', '$attrs', '$timeout', 'I18nService', '$interpolate', 'progressBarService', function ($scope, $element, $attrs, $timeout, I18nService, $interpolate, progressBarService) {
        ...
        $scope.displayProgressActive = 'active'; // Bootstrap active effect
													// for progress bar
        ...
        function calculateTimeUnits() {
          ...
          // plural - singular unit decision (old syntax, for backwards
			// compatibility and English only, could be deprecated!)
          ...
          // new plural-singular unit decision functions (for custom units and
        ...
        // determine initial values of time units and add AddSeconds
		// functionality
        if ($scope.countdownattr) {
        ...
        var tick = function tick() {
          ...
          // We are not using $timeout for a reason. Please read here -
			// https://github.com/siddii/angular-timer/pull/5
          $scope.timeoutId = setTimeout(function () {
              tick();
              // since you choose not to use $timeout, at least preserve
				// angular cycle two way data binding
          ...
          if(typeTimer !== null){
            ...
            if($scope.progressBar === 100){
              $scope.displayProgressActive = ''; // No more Bootstrap active
													// effect
            }
          }
        };
...
app.factory('I18nService', function() {
    ...
    I18nService.prototype.init = function init(lang, fallback) {
        ...
        // It should be handle by the user's application itself, and not inside
		// the directive
        // moment init
        // moment.locale(this.language);

        // human duration init, using it because momentjs does not allow
		// accurate time (
	 ...
	 * @param {int}
	 *            millis
	 * @returns {{millis: string, seconds: string, minutes: string, hours:
	 *          string, days: string, months: string, years: string}}
	 */
    I18nService.prototype.getTimeUnits = function getTimeUnits(millis) {
        var diffFromAlarm = Math.round(millis/1000) * 1000; // time in
															// milliseconds, get
															// rid of the last 3
															// ms value to avoid
															// 2.12 seconds
...
app.factory('progressBarService', function() {
  ...
  /**
	 ...
	 * @param {integer}
	 *            currentCountdown, where are we in the countdown
	 * @param {integer}
	 *            remainingTime, remaining milliseconds
	 * @param {integer}
	 *            endTime, end time, can be undefined
	 * @param {integer}
	 *            coutdown, original coutdown value, can be undefined
	 * 
  ...
  ProgressBarService.prototype.calculateProgressBar = function calculateProgressBar(startValue, remainingTime, endTimeAttr, coutdown) {
    ...
    displayProgressBar = 100 - displayProgressBar; // To have 0 to 100 and not
													// 100 to 0
    displayProgressBar = Math.round(displayProgressBar * 10) / 10; // learn
																	// more why
																	// :
